 ...
 *     // -or-
 *     return res.invalidRequest(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function invalidRequest(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.debug('Ran custom response: res.invalidRequest()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.debug(
      'Custom response `res.invalidRequest()` called with an Error:',
      optionalData
    );
